...
/*
  Setup multi-resolution registration stages
        n_inter - number of iterations
        img_smoothing_std - Guassian smoothing std used in downsampling
        downsample_factor - image downsampling factor
        learning_rate - used in optimization
        update_gaussian_sigma - used in displacement field update
        total_gaussian_sigma - used in the final displacement field 
        restrct - if 1, restrict the displacement field only in phase encoding direction (EPI)
        constrain - if 1, make the two displacement field 1, 2 to be the same
        metric: MSJac, CC, CCSK, CCJacs. weight.
*/
...
{
    // process up/down images to scale it to [0, 1]
    ...
    // For each registration stage, set up up_img/down_img/str_img
    for(int st=0;st<Nstg;st++)
    {
        bool redo=true;
        while(redo)
...
void DRBUDDI_Diffeo::Process()
{
    ...
    // set up_image, down_image, str_image for each stages
    SetImagesForMetrics();
    ...
    // Perform multi-resolution image registration
    for(int st=0;st< stages.size();st++)
    {
        ...
        (*stream)<<std::endl;
            ...
            // setup registration settings and perform image registration 
            DRBUDDIStage current_stage(&(stages[st]));
            current_stage.SetUpPhaseEncoding(up_phase_vector);
            current_stage.SetDownPhaseEncoding(down_phase_vector);
            current_stage.SetEstimateLRPerIteration(parser->getEstimateLRPerIteration());
