...
DRBUDDI::DRBUDDI(std::string uname,std::string dname,std::vector<std::string> str_names,json mjson)
{
    ...
    std::string json_PE= my_json["PhaseEncodingDirection"];      
...
{
    (*stream)<<"Enter DRBUDDI.Process()..."<<std::endl;
    (*stream)<<parser->getDRBUDDIStep()<<std::endl;

    // Step 0: get all images ready for distortion correction. 
    // The saved image files are blip_up_b0.nii, blip_down_b0.nii.
    if(parser->getDRBUDDIStep()==0)
        Step0_CreateImages();

    (*stream)<<"Finished Step0_CreateImages..."<<std::endl;

    // Step 1: perform rigid image registration if enabled
    ...
    (*stream)<<"Finished Step1_RigidRegistration..."<<std::endl;

    // Step 2: Perform diffeo image registration
    Step2_DiffeoRegistration();
    (*stream)<<"Finished Step2_DiffeoRegistration..."<<std::endl;

    // Step 3: save output
    Step3_WriteOutput();
    (*stream)<<"Finished Step3_WriteOutput..."<<std::endl;
}


void DRBUDDI::Step0_CreateImages()
{
    // Juan comment the two lines
    ...
    this->b0_up=readImageD<ImageType3D>(this->up_nii_name);
    this->b0_down=readImageD<ImageType3D>(this->down_nii_name);

    // if gradient field map is provided, perform gradient unwarping 
...
DRBUDDI::RigidTransformType::Pointer DRBUDDI::RigidDiffeoRigidRegisterB0DownToB0Up(ImageType3D::Pointer b0_up_image, ImageType3D::Pointer b0_down_image, std::string mtype, ImageType3D::Pointer & initial_corrected_b0)
{
    ...
    // 1. blip up, blip down rigid image registration
    ...
    // 2. initial diffeomorphic registration
    ...
    // 3. ReStarting initial rigid registration
...
/*
    1) Create QuadImages: blip_up_b0_quad.nii, blip_down_b0_quad.nii
    2) Perform Rigid Diffeo Rigid Registeration for Up, down images 
    3) Regid registration all structural images to blip_up_b0
*/
void DRBUDDI::Step1_RigidRegistration()
{
    ...
    // Perform rigid + diffeo registration of b0_down to b0_up.
    ...
    for(it.GoToBegin(); !it.IsAtEnd(); ++it) // make sure all pixels with value >= 0
    {
        if(it.Get()<0)
            it.Set(0);
    }
    ...
    for(int str=0;str<Nstr;str++)
    {
        ...
        ImageType3D::Pointer str_img= create_mask(str_img_orig);  // Juan disable the bet brain masking due to compile issues

        int Do_RigidTransform = 0;  // Juan disable the regid registration
        RigidTransformType::Pointer rigid_trans1=nullptr;
        RigidTransformType::Pointer rigid_trans2=nullptr;
        if (Do_RigidTransform)
        {
            rigid_trans1= RigidRegisterImagesEuler( initial_corrected_b0,  str_img, "CC", parser->getRigidLR());
            rigid_trans2= RigidRegisterImagesEuler( initial_corrected_b0,  str_img, "MI", parser->getRigidLR());
        }
        else
        {
            rigid_trans1 = RigidTransformType::New();
            rigid_trans2 = RigidTransformType::New();
            rigid_trans1->SetIdentity();
            rigid_trans2->SetIdentity();
        }
