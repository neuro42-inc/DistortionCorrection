...
/*
Calculate the deformation field updates based on MSJac

Inputs: up_img, down_img - blip-up blip-down images
        def_FINV, def_MINV
        updateFieldF, updateFieldM - calculated updated change in deformation fields for this voxel position (calculated in the code)
        phase_vector
                          ...
                          vnl_vector<double> phase_vector,itk::GaussianOperator<float,3> &current_Gaussian_operator)
{
    ...
    // sum up m_MetricImage to get the metric 
    double value = 0;
