...
/*
    Create virtual image for image registration. In multi-resolution registration stages, downsampling is used. The virtual image created has 
    corresponding image properties (Origin, Spacing, Direction, Regions). 
*/
...
/*
    Preprocess image and displace field for image registation
    1) create virtual image (spacing, origin, direction, region)
    2) create def_FINV, def_F, def_MINV, def_M if NULL and they have the same image properties (spacing, origin, direction, region) as the virtual image
    3) if def_FINV, def_FINV not NULL (from the previous stage of image registration), resample them to match virtual image
    4) if def_F, def_M is NULL, invert def_FINV, def_MINV to get them
    5) Get downsampled up/down/str images. First perform Gaussian filtering then downsampling to reduce aliasing caused by downsampling.
        resampled_smoothed_up_images
        resampled_smoothed_down_images
        resampled_smoothed_str_images
*/
void DRBUDDIStage::PreprocessImagesAndFields()
{
    ...
    // Perform Gaussian smoothing before downsampling
...
/*
  Run multi-stages of image registration 
*/
void DRBUDDIStage::RunDRBUDDIStage()
{
    ...
    while( iter++ < this->settings->niter && !converged )
    {
        ...
        // 1) Use the displacement field to warp up/down images 
        // 2) calculate metrics and its gradient deveriates of displacement field 
        // 3) get the displacement field descent
        ...
        {   
            // Use the displacement map to get warped images
            ...
            // Compute the metrics and updated displacement map
        ...
        // restrict the displacement field only at phase encoding direction (EPI)
        ...
        // To get the best_lr to update displacement field
        ...
        float beta_f=0,beta_m=0;
            ...
            // choose the best updated displacement field from different lr
            ...
            {
                // Scale the displacement field by lr * gradient direction 
...

                {
                    ...
                    // 1) Get composed fields, 2) smooth it, 3) get inverse displacement field
                    ...
                    // constrain the field if needed and then get whole inverse displacement field, then apply to image and calculate metrics
        ...
        // use the best lr to displacement map update (lr * gradient descent)
        ScaleUpdateField(conjugateFieldF,best_lr);
        ScaleUpdateField(conjugateFieldM,best_lr);

        // 1) get displacement field after updates 
        // 2) Gaussian smooth displacement map, 
        // 3) Invert the displacement map and get def_FINV, def_MINV 
        // 4) Constrian inverted displacement map pairs
        // 5) update def_F, def_M, def_FINV, def_MINV
        {
            CurrentFieldType::Pointer f2midtmp=nullptr,f2midtmp_inv=nullptr;
            CurrentFieldType::Pointer m2midtmp=nullptr,m2midtmp_inv=nullptr;
