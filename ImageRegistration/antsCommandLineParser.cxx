...
void CommandLineParser::PrintMenu( std::ostream& os, Indent indent, int module_to_print, bool printShortVersion ) const
{
    if(module_to_print==0)
    {
        /*
      os << std::endl;
      os << "COMMAND: " << std::endl;
  ...
  for( it = this->m_Options.begin(); it != this->m_Options.end(); ++it )
  {
      if( (*it)->GetModule()== module_to_print)
      {
